...
GeoXML3.parser = function (options) {
  ...
  window.google.maps.MVCObject.call(this);
  ...
  if (!parserOptions.infoWindow && parserOptions.singleInfoWindow && !!window.google && !!google.maps)
    parserOptions.infoWindow = new window.google.maps.InfoWindow();
  ...
  var render = function (responseXML, doc) {
    ...
    if (!responseXML || responseXML === 'failed parse') {
      ...
      for (var pm = 0; pm < placemarkNodes.length; pm++) {
            ...
            if (!GeometryNodes[gn].parentNode ||
              ...
              switch (Geometry) {
                case 'Point':
                  ...
                  if (!!window.google && !!google.maps)
                    placemark.latlng = new window.google.maps.LatLng(placemark.Point.coordinates[0].lat, placemark.Point.coordinates[0].lng);
                  pathLength = 1;
                  break;
                case 'LinearRing':
                  // Polygon/line
        ...
        if (!!window.google && !!google.maps) {
          if (placemark.Point) {
            if (!!window.google && !!google.maps) {
              doc.bounds = doc.bounds || new window.google.maps.LatLngBounds();
      ...
      for (i = 0; i < groundNodes.length; i++) {
        ...
        if (!!window.google && !!google.maps) {
          doc.bounds = doc.bounds || new window.google.maps.LatLngBounds();
          doc.bounds.union(new window.google.maps.LatLngBounds(
            new window.google.maps.LatLng(groundOverlay.latLonBox.south, groundOverlay.latLonBox.west),
            new window.google.maps.LatLng(groundOverlay.latLonBox.north, groundOverlay.latLonBox.east)
          ));
        }
          ...
          if (!!parserOptions.createOverlay) {
            ...
            if (!!doc) {
              ...
              if (!!window.google && !!google.maps && doc.reload) {
                var overlayBounds = new window.google.maps.LatLngBounds(
                  new window.google.maps.LatLng(groundOverlay.latLonBox.south, groundOverlay.latLonBox.west),
                  new window.google.maps.LatLng(groundOverlay.latLonBox.north, groundOverlay.latLonBox.east));
    ...
    if (!!doc.bounds && !!window.google && !!google.maps) {
      doc.internals.bounds = doc.internals.bounds || new window.google.maps.LatLngBounds();
    ...
    if (doc.internals.remaining === 0) {
      ...
      window.google.maps.event.trigger(doc.internals.parser, 'parsed');
    }
  };
  ...
  var processStyleID = function (style) {
    if (!!window.google && !!google.maps) {
      var zeroPoint = new window.google.maps.Point(0, 0);
      if (!!style.href) {
        var anchorPoint;
        var markerRegEx = /\/(red|blue|green|yellow|lightblue|purple|pink|orange|pause|go|stop)(-dot)?\.png/;
        if (markerRegEx.test(style.href)) {
          // bottom middle
          anchorPoint = new window.google.maps.Point(16 * style.scale, 32 * style.scale);
        } else {
          anchorPoint = new window.google.maps.Point(16 * style.scale, 16 * style.scale);
        }
        // Init the style object with a standard KML icon
        style.icon = {
          url: style.href,
          size: new window.google.maps.Size(32 * style.scale, 32 * style.scale),
          origin: zeroPoint,
          // bottom middle
          anchor: anchorPoint,
          scaledSize: new window.google.maps.Size(32 * style.scale, 32 * style.scale)
        };
        // Look for a predictable shadow
        var stdRegEx = /\/(red|blue|green|yellow|lightblue|purple|pink|orange)(-dot)?\.png/;
        var shadowSize = new window.google.maps.Size(59, 32);
        var shadowPoint = new window.google.maps.Point(16, 32);
  ...
  var createMarker = function (placemark, doc) {
    ...
    var markerOptions = GeoXML3.combineOptions(parserOptions.markerOptions, {
      ...
      position: new window.google.maps.LatLng(placemark.Point.coordinates[0].lat, placemark.Point.coordinates[0].lng),
    ...
    if (!parserOptions.suppressInfoWindows) {
      var infoWindowOptions = GeoXML3.combineOptions(parserOptions.infoWindowOptions, {
        ...
        pixelOffset: new window.google.maps.Size(0, 2)
      });
      if (parserOptions.infoWindow) {
        marker.infoWindow = parserOptions.infoWindow;
      } else {
        marker.infoWindow = new window.google.maps.InfoWindow(infoWindowOptions);
      }
  ...
  var createPolyline = function (placemark, doc) {
    ...
    for (var j = 0; j < placemark.LineString.length; j++) {
      ...
      for (var i = 0; i < coords.length; i++) {
        var pt = new window.google.maps.LatLng(coords[i].lat, coords[i].lng);
        path.push(pt);
        bounds.extend(pt);
      }
      paths.push(path);
    ...
    if (paths.length > 1) {
      ...
      p = new window.google.maps.Polyline(polyOptions);
    }
    p.bounds = bounds;
    ...
    if (!parserOptions.suppressInfoWindows) {
      ...
      if (parserOptions.infoWindow) {
        ...
        p.infoWindow = new window.google.maps.InfoWindow(infoWindowOptions);
      }
      p.infoWindowOptions = infoWindowOptions;
      // Infowindow-opening event handler
      window.google.maps.event.addListener(p, 'click', function (e) {
        p.infoWindow.close();
  ...
  var createPolygon = function (placemark, doc) {
    ...
    for (var polygonPart = 0; polygonPart < placemark.Polygon.length; polygonPart++) {
      ...
      for (j = 0; j < placemark.Polygon[polygonPart].outerBoundaryIs.length; j++) {
        ...
        for (i = 0; i < coords.length; i++) {
          pt = new window.google.maps.LatLng(coords[i].lat, coords[i].lng);
          path.push(pt);
          bounds.extend(pt);
        }
        paths.push(path);
    ...
    var p = new window.google.maps.Polygon(polyOptions);
    p.bounds = bounds;
    if (!parserOptions.suppressInfoWindows) {
      var infoWindowOptions = GeoXML3.combineOptions(parserOptions.infoWindowOptions, {
        content: '<div class="geoxml3_infowindow"><h3>' + placemark.name +
